...
def camel_to_snake_case(name):
    ...
    inbetween = re.compile(r'''
                            (
                             (?<=[a-z])()(?=[A-Z])      # between a lowercase and  an uppercase letter
                             |                   # or
                             (?=.)()(?=[A-Z][a-z])        # in front of an uppercase followed by a lowercase 
                            )
                            ''', re.VERBOSE)
...
def not_both_titles(names_string):
    ...
    pat = re.compile(r'''
                     (?:[Pp]rof|[Dd]oc)\.[\w .]+?,\sPh\.D\.                        # Captures everything that don't fit to a non-capturing group 0
                     |                                                             # Or
                     \b((?:[Pp]rof\.|[Dd]oc\.)?[\w .]+(?:,\sPh\.D\.)?)             # Captures everything else to a group 1
                      ''', re.X)
